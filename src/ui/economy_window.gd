	...
	for fac in matchingFacilities:
		...
		if _original:
			...
			OUI.Card(rowBtn, fac.Name(), OUI.Mini("facilities", fac.Def.Id if fac.Def != null else fac.Family()),
				Color.RED if fac.IsDamaged else Color.WHITE, OUI.SideColor(GameSettings.PlayerFaction), "", null, false)
	...
	for line in seen:
		...
		if _original:
			...
			OUI.Card(row, str(line), OUI.Mini("facilities", seenDef.Id if seenDef != null else family),
				Color.WHITE, OUI.SideColor(GameSettings.PlayerFaction), "", null, false)
