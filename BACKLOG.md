...
| 37 | Remaining family-name reads, all display-side: the Defense Facility Status window's names and icons; `mine` / `refinery` / `shipyard` ids in `economy.gd:99`, `agent_droid.gd:78`, `intel_manager.gd:398` (same ids in both packs, so not a blocker) | the three above | **Window done** (TeeJ, 2026-09-22): roles, the family's tier-1 display name, `Terms` for the shield row and the Defenses tags. The three id reads remain, not a blocker |
