...
| `map.x`, `map.y` | **★ DECIDED (TeeJ, 2026-09-22): pixels of the pack's `map_image` at its native size.** The map view draws the picture fitted into its frame and places every marker at `coordinate × scale`, so a picture and its regions line up at any size. (Before this the Star Wars picture was baked into `Main.tscn` at one fixed rectangle and the coordinates were the original's screen space; they were converted, pixel = (x+5, y−110) / 1.6729.) |
...
| 53 | **The map picture comes from the pack.** `GalaxyMap` loads `map_image`, fits it into its frame and scales every marker with it; the Star Wars picture baked into `Main.tscn` is gone and `map.json` coordinates are picture pixels (§4). Star Wars coordinates converted (no simulation reads them) | TeeJ, 2026-09-22: the WWII pack played over the Star Wars galaxy; a pack's own map has to show |
