...
| `map_image_rect` | **★ 2026-09-22.** Where the picture sits in the pack's map coordinate space, `[x, y, w, h]`. The map view scales the space so this rectangle fills its frame and draws the picture into it; markers follow at the same scale. Absent: the picture's own pixels are the space. Star Wars: `[-5, 110, 1070.67, 803]`, exactly where `Main.tscn` used to bake the picture, so nothing moved. WWII: `[0, 0, 700, 420]` for a 1750×1050 picture. |
