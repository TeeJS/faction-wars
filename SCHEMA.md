...
| `movies` | **★ 2026-09-26 (TeeJ, docs/cutscenes-plan.md).** Which movie plays at which engine event: `{event: movie}` or `{event: [movie, ...]}`, played in order, each skipped by a click or Esc. A movie is `"<art set>:<path>.ogv"` - the player's own movies file, converted by the Faction Wars Exporter and imported like the art set (Ogg Theora, the only kind the engine plays) - or a file the pack ships. The events are the engine's, named by role: `launch` (once a run, when the Cockpit first opens: manual p022's "introductory graphics"), `credits` (View credits; without the movie the credits window, as before), `system_destroyed`, `superweapon_sabotaged`, and `start.` / `victory.` / `defeat.` / `headquarters_lost.` + a faction id (`headquarters_lost.<side>` plays for everyone when that side's headquarters is captured or destroyed, before the war's end when it ends it). Optional; a movie that is not there plays nothing, so a pack is never broken by a missing movies file (validation rule 23 checks the shape). Star Wars: `launch` 000 + 001, `credits` 005, `start.alliance` 003, `start.empire` 004, `headquarters_lost.alliance` 102, `headquarters_lost.empire` 103, `system_destroyed` 101, `superweapon_sabotaged` 104, `victory.alliance` 105, `defeat.alliance` 106, `defeat.empire` 107, `victory.empire` 108 - the moments from the manual, the movies themselves and the Rebellion 2 remake's player (its boot plays "intro" then "opening-crawl"; each side has an intro, a headquarters-destroyed movie, a victory and a defeat); which side 003/004 and 102/103 belong to is by what they show. 201/202 (a Death Star trench run) wait for a moment to play at. |
| `music` | **★ 2026-09-26 (TeeJ, docs/music-plan.md).** Which track plays at which moment: `{moment: track}`. A track is `"<art set>:<path>.ogg"` (Ogg Vorbis; the exporter puts the original's score in the art set as `music/<nnn>.ogg`) or a file the pack ships. The moments are the engine's: `menu` (the Shuttle Cockpit, looped while it is up; it stops when a game starts). Game Options' Play Music and music volume drive it; a movie holds it. Optional; a track that is not there plays nothing (validation rule 24 checks the shape). Star Wars: `menu` 300 (The Battle of Endor I - open-rebellion's notes and the Rebellion 2 remake agree). The in-play playlist and the battle alert wait on the music plan's phase 0. |
...
24. ✅ `pack.json` `music`, when given, is an object whose keys are known
    moments (`menu`), each naming one track: a file the pack ships, or an
    `.ogg` in a declared art set (not required to be imported).
