...
23. ✅ `pack.json` `movies`, when given, is an object whose keys are known events
    ...
    `start.` / `victory.` / `defeat.` + a faction id in `factions.json`), each
    naming one or more movies: a file the pack ships, or an `.ogv` in a declared
    art set (not required to be imported).
