...
| Item | What shipped |
|---|---|
| **Map layout** | **Real geography on the 1941 map** (since 2026-09-22; the Phase C board is gone). Each region sits where the map puts it, via a mapping fitted on 90 cities read off the map (cubic in lon/lat, 18 px RMS). The Central Pacific is unwrapped onto the map's right copy so that theatre does not span the whole width. **Europe is crowded**: 45 regions in roughly 200×250 px of a 1750-wide picture, 12–25 px apart at in-game scale, and its sector boxes overlap — an inset, or fewer regions, is the open question. |
