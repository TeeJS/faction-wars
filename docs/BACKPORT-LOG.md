...
## 21. Popular Support label takes the player's faction color; Leave Game on a real MP drop     port (room work)
Kind:      two UI fixes. (a) label color - a real bug present in the C# too (BACKPORT). (b) MP waiting-box timing - port-only (web multiplayer is not in the C#).
Files:     `src/ui/gid_bar.gd` (the GID active-mode label - "Popular Support" etc. - takes `GameSettings.PlayerFaction.FactionColor` instead of a hardcoded green, so it is red for the Rebellion, green for the Empire, and correct for any faction pack; null-guard keeps the old green as a fallback), `src/ui/game_manager.gd` (`_MpWatch`: `_wasWaiting` timestamps the wait once on the false->true transition so closing/reopening the box no longer restarts the Leave-Game clock; `_leaveBtn.visible = session.opponent_gone or (now - _waitingSince > LeaveAfterMs)`, so Leave appears immediately on a real disconnect and after a minute on a pause)
Source:    TeeJ, room AM-D9LA9WHG968KAC8VW8W33BG9QR: "'Popular Support' should be red when playing the rebellion" and "sometimes I don't get the exit option when my opponent drops/leaves ... there should be an option to drop my end as well." Label bug: `frontend/GidBar.cs:43` hardcodes `new Color(0.45f,1.0f,0.45f)` (Empire green) for every faction and the port copied it; the pack already defines alliance #ff0000 / empire #00ff00 (`packs/star-wars-rebellion/factions.json`). Drop bug: `_MpWatch` reset `_waitingSince` every time the box became visible, so an X-close-and-repop restarted the 60s timer, and it gated Leave on the 60s delay even for a real `opponent_gone`. Found and diffed by Doof, verified by DoofTest and me against the code; approved by TeeJ ("go").
Hashes:    unchanged (UI-only: label color and button visibility; no simulation state). Gate: `tests/ui_compile.gd` 52 of 52. The behavioral checks (label red as the Rebellion; Leave Game immediate on a drop and surviving close/reopen) are TeeJ's live test - no headless gate simulates a live opponent drop or asserts a label color.
BACKPORT:  (a) only. Apply the faction-color fix to `frontend/GidBar.cs:43` in sol-conflict-revolution (drive the label from the player faction's color, not the hardcoded green). (b) is web-MP code with no C# counterpart.

## 22. Fleets explore: arrival at a system you do not hold gives a Reconnaissance-grade sighting     port (branch fleet-explore)
Kind:      rule (missing feature)
Files:     `src/game/strategic_tick_manager.gd` (PROCESS FLEET MOVEMENT: on the day a fleet arrives at a world its faction does not control, `IntelManager.Capture(fleet.Faction, landing, CurrentDay, IntelManager.ReconnaissanceCategories)`, which also charts the system; one `EventBus.BroadcastChanged()` after the loop so the map recolours), `tests/fleet_explore.gd`, `GAMEPLAY.md` (section 13, "Fleets can explore": the confidence table and the game/code table), `tests/fixtures/ai-rebuild-seed12345-100d.log`
Source:    `backend/StrategicTickManager.cs`, fleet movement: the C# has the same gap (arrival sets Attached/Status and nothing else; `IntelManager.Capture` is called only from day zero, informants and missions). Same one call, same place. ARRIVAL ONLY - no source says a fleet that stays in orbit keeps the sighting fresh, so that is not implemented (GAMEPLAY.md records it as Unknown and what would settle it). The original's "Fleet Arrives at |" message (TEXTSTRA.DLL) is a separate gap, not in this entry.
Hashes:    this change moves them only when an AI fleet arrives somewhere it did not already know: measured pre/post over 300 days, seed 12345 Medium first differs on **day 209**; seed 12345 Easy, seed 12345 Hard and seed 777 Medium do **not differ** in 300 days; the 100-day gate run is byte-identical pre/post. The fixture was regenerated anyway because it was ALREADY STALE on main: 68 of 101, first divergence day 69, caused by commit 129e00a (the defender's "Enemy Mission Foiled" notice adds a message, and `GameSignature` hashes the message count) - confirmed by silencing `_tell_defender`, which restored 101 of 101 against the old fixture. New fixture: two runs byte-identical, 101 of 101.
Sources:   manual p121 (PDF p119) "a fleet can explore an unexplored system ... you learn the same information ... that you do from a Recon mission ... except any characters or SpecForces that may be present and information concerning current manufacturing"; TEXTSTRA.DLL advisor text, both sides ("employ our capital ships for exploration") - Confirmed. Charted-but-not-held systems: manual p069 ("only apparent if you send a fleet or a mission to a system to investigate") - single-source.
